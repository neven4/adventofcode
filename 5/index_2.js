...
try {  
    ...
    preparedCoords.forEach(lineData => {
        ...
        if (Math.abs(x1 - x2) === Math.abs(y1 - y2)) {
            const countData = x2 > x1 ? {from: x1, to: x2, isUp: y2 > y1, y: y1} : {from: x2, to: x1, isUp: y1 > y2, y: y2}

            for (let i = countData.from; i <= countData.to; i++) {
                const xCoord = i
                const yCoord = countData.y
                const coordData = `${xCoord}_${yCoord}`

                setCoordData(coordData)

                countData.y = countData.isUp ? countData.y + 1 : countData.y - 1
            }
